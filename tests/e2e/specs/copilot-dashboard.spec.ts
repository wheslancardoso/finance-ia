...
test.describe('Modo Copiloto de IA (Modo Jarvis) e Simulações no Chat', () => {
  ...
  test('deve abrir o painel lateral do Copiloto, carregar histórico/memórias do banco, enviar pergunta, obter resposta e simular gasto', async ({ page, isMobile }) => {
    ...
    // Teto semanal inicial deve ser baseado no saldo projetado corrigido pelo SSOT
    await expect(page.getByTestId('survival-ceiling-value')).toContainText(/320/);
    ...
    await expect(page.getByTestId('survival-ceiling-value')).toContainText(/300/);
