...
import { ArrowLeft, Repeat, Play, Pause, CreditCard, Wallet } from 'lucide-react-native';
        ...
        <View className="mb-8">
          ...
          {activeRecurring.length === 0 ? (
            ...
            activeRecurring.map((item, index) => (
              <RecurringItem key={item.id} item={item} index={index} onToggle={toggleStatus} />
            ))
          )}
        </View>
        ...
        {pausedRecurring.length > 0 && (
          <View className="mb-8">
            ...
            {pausedRecurring.map((item, index) => (
              <RecurringItem key={item.id} item={item} index={index + 10} onToggle={toggleStatus} />
            ))}
          </View>
        )}
...
function RecurringItem({ item, index, onToggle }: { item: any, index: number, onToggle: any }) {
  ...
  return (
    <MotiView
      ...
      className={`flex-row items-center justify-between p-5 rounded-[24px] mb-4 border ${
