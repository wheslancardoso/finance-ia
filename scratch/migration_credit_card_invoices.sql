-- Migração: Criação de faturas de cartão de crédito e lógicas de triggers

-- 1. Criação da tabela credit_card_invoices
CREATE TABLE IF NOT EXISTS credit_card_invoices (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    account_id UUID REFERENCES accounts(id) ON DELETE CASCADE,
    reference_month VARCHAR(7) NOT NULL, -- Ex: '2026-06'
    closing_date DATE NOT NULL,
    due_date DATE NOT NULL,
    amount_cents BIGINT DEFAULT 0,
    paid_amount_cents BIGINT DEFAULT 0,
    status VARCHAR(20) DEFAULT 'OPEN', -- 'OPEN', 'CLOSED', 'PAID', 'PARTIALLY_PAID'
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(account_id, reference_month)
);

-- 2. Adicionar coluna invoice_id em transactions
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS invoice_id UUID REFERENCES credit_card_invoices(id) ON DELETE SET NULL;

-- 3. Função e Trigger para associar a transação a uma fatura
...
BEGIN
    ...
    IF v_base_date <= v_test_closing_date THEN
        -- Comprou antes ou no dia do fechamento (Cai na fatura deste ciclo)
        v_invoice_month := v_test_closing_date;
    ELSE
        -- Comprou após o fechamento, cai no próximo ciclo
    ...
    -- O mês de referência da fatura será o mês/ano do vencimento ou do fechamento. 
    -- Faturas de cartão geralmente tem o "Mês de Referência" como o mês em que vence.
...
-- 4. Função e Trigger para manter o amount_cents da fatura atualizado
...
-- 5. Lógica para transações já existentes
-- Força a trigger a rodar para todas as transações de cartão de crédito atuais, associando-as às suas faturas recém-criadas.
DO $$
BEGIN
    UPDATE transactions 
