...
import { Calculator, AlertTriangle, CheckCircle2, XCircle, TrendingDown, TrendingUp, PlusCircle, Sparkles, ChevronDown, ChevronUp } from "lucide-react";
...
export default function SpendingSimulator({ onSimulate, targetDate }: SpendingSimulatorProps) {
  ...
  const [sandboxSimulations, setSandboxSimulations] = useState<any[]>([]);
  ...
  React.useEffect(() => {
    if (onSimulate) {
      const activeSandbox = sandboxSimulations.filter(s => s.active);
      const cleanValue = amount.replace(/\./g, "").replace(",", ".");
      const valueCents = Math.round(parseFloat(cleanValue) * 100);
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-[32px] p-5 h-full flex flex-col shadow-2xl relative overflow-hidden">
      ...
      <div className="space-y-4 relative z-10 flex-1">
            ...
            <div className="space-y-2 mt-3">
              <button 
                onClick={() => {
                  ...
                  setSandboxSimulations(prev => [...prev, newSim]);
                ...
                className="w-full py-2.5 rounded-lg font-black text-[9px] uppercase tracking-widest bg-violet-600 text-white hover:bg-violet-500 transition-all flex items-center justify-center gap-1.5 shadow-lg shadow-violet-600/10 border border-violet-500/20 cursor-pointer"
              >
                <Sparkles className="w-3.5 h-3.5 text-violet-200 animate-pulse" />
                Simular no Sandbox Coletivo
              </button>
        ...
        {/* Sandbox Coletivo Panel (Expansível) */}
        {sandboxSimulations.length > 0 && (
          <div className="mt-4 pt-4 border-t border-white/5 space-y-2 text-left">
            <button
              onClick={() => setIsSandboxOpen(!isSandboxOpen)}
              className={cn(
                "w-full flex items-center justify-between py-1.5 px-2.5 rounded-xl transition-all text-[9px] font-black uppercase tracking-widest cursor-pointer",
                isSandboxOpen 
                  ? "bg-violet-600/10 border border-violet-500/20 text-violet-400" 
                  : "bg-white/[0.01] hover:bg-white/[0.03] border border-white/5 text-white/40 hover:text-white/60"
              )}
            >
              <div className="flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-violet-400 animate-pulse" />
                <span>Sandbox Coletivo ({sandboxSimulations.length})</span>
              </div>
              <div className="flex items-center gap-2">
                <span 
                  onClick={(e) => {
                    e.stopPropagation();
                    setSandboxSimulations([]);
                  }}
                  className="text-[8px] text-red-400/80 hover:text-red-400 hover:underline transition-all cursor-pointer font-bold animate-in fade-in"
                >
                  Limpar Todos
                </span>
                {isSandboxOpen ? (
            ...
            {isSandboxOpen && (
              <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1 custom-scrollbar pt-1 animate-in fade-in slide-in-from-top-1 duration-200">
                {sandboxSimulations.map((sim) => (
                  <div 
                    key={sim.id} 
                    className={cn(
                      "flex items-center justify-between border rounded-xl p-2.5 transition-all",
                      sim.active 
                        ? (sim.type === "INCOME" ? "bg-emerald-500/5 border-emerald-500/10 text-emerald-400" : "bg-red-500/5 border-red-500/10 text-red-400")
                        : "bg-white/[0.01] border-white/5 text-white/40"
                    )}
                  >
                    <div className="min-w-0 flex-1">
                      <p className={cn("text-[9px] font-bold truncate", sim.active ? "text-white" : "text-white/30")}>
                        {sim.description}
                      </p>
                      <p className="text-[7px] font-black uppercase tracking-widest text-white/20 mt-0.5">
                        {sim.type === "INCOME" ? "Receita" : "Gasto"} • {sim.installments}x
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 shrink-0 ml-3">
                      ...
                      {/* Glassmorphic Switch Toggle */}
                      <button
                        onClick={() => {
                          setSandboxSimulations(prev => prev.map(s => s.id === sim.id ? { ...s, active: !s.active } : s));
                        }}
                        className={cn(
                          "w-7 h-4 rounded-full relative transition-all duration-300 border focus:outline-none cursor-pointer",
                          sim.active 
                            ? (sim.type === "INCOME" ? "bg-emerald-500/20 border-emerald-500/30" : "bg-red-500/20 border-red-500/30")
                            : "bg-white/5 border-white/10"
                        )}
                      >
                        <div 
                          className={cn(
                            "w-2.5 h-2.5 rounded-full absolute top-1/2 -translate-y-1/2 transition-all duration-300 shadow",
                            sim.active 
                              ? (sim.type === "INCOME" ? "left-3.5 bg-emerald-400" : "left-3.5 bg-red-400")
                      ...
                      <button 
                        onClick={() => {
                          setSandboxSimulations(prev => prev.filter(s => s.id !== sim.id));
                        }}
                        className="p-1 rounded-lg bg-white/5 text-white/40 hover:bg-red-500/10 hover:text-red-400 transition-colors border border-white/5 cursor-pointer"
                      >
                        <XCircle className="w-3.5 h-3.5" />
                      </button>
                    </div>
