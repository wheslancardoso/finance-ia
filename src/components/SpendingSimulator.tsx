  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-[32px] p-5 h-full flex flex-col shadow-2xl relative overflow-hidden">
      ...
      <div className="space-y-4 relative z-10 flex-1">
        ...
        {/* Sandbox Coletivo Panel */}
        {sandboxSimulations.length > 0 && (
          <div className="mt-4 pt-4 border-t border-white/5 space-y-3 text-left">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-violet-400 animate-pulse" />
                <h4 className="text-[9px] font-black text-white/80 uppercase tracking-widest">Sandbox Coletivo ({sandboxSimulations.length})</h4>
              </div>
              <button 
                onClick={() => setSandboxSimulations([])}
            ...
            <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1 custom-scrollbar">
              {sandboxSimulations.map((sim) => (
                <div 
                  key={sim.id} 
                  className={cn(
                    "flex items-center justify-between border rounded-xl p-2.5 transition-all animate-in fade-in duration-200",
                    sim.active 
