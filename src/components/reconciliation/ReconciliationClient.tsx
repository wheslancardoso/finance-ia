...
export function ReconciliationClient({ initialAccounts, initialTransactions }: ReconciliationClientProps) {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white/5 border border-white/10 rounded-[32px] p-6 space-y-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 className="text-xl font-black text-white tracking-tight">Qual Conta?</h2>
      ...
      <div className="bg-violet-600/10 border border-violet-500/30 rounded-[32px] p-8 space-y-6">
        <div className="flex items-center gap-4">
          <div className="p-4 bg-violet-600 rounded-2xl shadow-[0_0_20px_rgba(124,58,237,0.4)]">
            <LockKeyhole className="w-8 h-8 text-white" />
          </div>
          <div>
            <h2 className="text-2xl font-black text-white tracking-tight">Lacrar Mês</h2>
            <p className="text-sm font-medium text-white/60">Informe o saldo exato que está no app do banco agora</p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-end gap-6">
          <div className="flex-1 w-full space-y-2">
            ...
            <input 
              ...
              className="w-full bg-black/40 border border-white/20 rounded-2xl px-5 py-4 text-white text-xl font-mono focus:outline-none focus:border-violet-500 transition-colors"
            />
          </div>
          ...
          <div className="flex-1 w-full p-4 bg-black/20 rounded-2xl border border-white/5 flex flex-col justify-center">
            ...
            {differenceCents !== null ? (
              <span className={`text-2xl font-black tabular-nums ${differenceCents === 0 ? 'text-emerald-400' : 'text-amber-400'}`}>
          ...
          <button
            ...
            className="w-full md:w-auto px-8 py-4 bg-violet-600 hover:bg-violet-500 disabled:bg-violet-600/50 text-white rounded-2xl font-black transition-all active:scale-95 flex items-center justify-center gap-3"
          >
            {isReconciling ? (
        ...
        {differenceCents !== null && differenceCents !== 0 && (
          <div className="text-sm font-medium text-amber-400/80 bg-amber-500/10 p-4 rounded-xl">
